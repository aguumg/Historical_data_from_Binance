...
class BinanceClient(object):
    ...
    def get_all_binance_from_to(self, symbol, initial_date, days_ahead, freq, save=False):
        binsizes = {"1m": 1, "5m": 5, "10m": 10, "15m": 15, "1h": 60, "6h": 360, "12h": 720, "1d": 1440}
        filename = '%s-%s-data.csv' % (symbol, freq)
        ...
        newest_point = oldest_point+timedelta(days=days_ahead)
...
symbol_btc = 'BTCBUSD'
freq = '1h'
initial_date = "10 Mar 2020"
_binance_client_ = BinanceClient(config['binance_client'])
eth_historical = _binance_client_.get_all_binance_from_to(symbol=symbol_eth, freq=freq,
                              initial_date=initial_date, days_ahead=5, save=True)
btc_historical = _binance_client_.get_all_binance_from_to(symbol=symbol_btc, freq=freq,
                              initial_date=initial_date, days_ahead=5, save=True)
